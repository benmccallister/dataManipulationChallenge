...
shipments_with_lateness = (
    ...
    .assign(
        is_late=lambda df: df['actualShipDate'] > df['plannedShipDate'],
        days_late=lambda df: (df['actualShipDate'] - df['plannedShipDate']).dt.days,
        lateStatement=['is_late'], "Darn Shipment is Late", "Shipment is on Time")
    )
)
```
...
> Question1:
The dtype is datetime64[ns] which can be found by looking at the code which has .dt at the end or by running the following code: print(shipments_df['plannedShipDate'].dtype). It is important that they have the same data type for easy comparison. As evidenced by the days_late calculation, we can actually subtract the planned ship date from the actual ship date to find how late the shipment was. We would not be able to calculate the differnce of different data types.
Question 2:
# Compare as strings
...
# Compare as dates
print(pd.to_datetime("04-11-2025") - pd.to_datetime("05-20-2024"))
# Output: 355 days 00:00:00
...
print(shipments_with_lateness['lateStatement'].head())
```
...
> Question 1:
Both methods filter rows where is_late is True. The only difference is how you write it: .query('is_late == True') is more readable and easier to write.
...
> Question 1:
Drop is used to remove specific unwanted columns. Filter is used to keep specific columns. You would choose to drop if you only want to drop a few columns. You would choose to filter if you only want to keep a few columns.

Question 2:
.dropna() without specifying a subset would drop any row that has a missing value in any column. By specifying a subset, only rows that have missing values in the specified columns will be removed. You may want to be more selective if there are certain columns whose values are less important. These columns may have many rows with missing values that would lead too many rows to be removed if .dropna() was used indiscriminately.
...
> Question 1: 
ascending=False sorts from highest to lowest. ascending=True sorts from lowest to highest. 
To sort by multiple columns:
```{python}
shipments_with_lateness.sort_values(['is_late', 'days_late'], ascending=[False, False])
```
Question 2:
Before running a sort function like the one above, the data is indexed from 0 to the end of the data set. After sorting, since that data is now in order, the numbers indicating the index positions are out of order, by reseting the index, the rows are renumbered according to their new positions rather than their original ones.
...
> Question 1:
The boolean function assigns the number 1 to true and the number 0 to false, so by summing boolean values, you get the sum of the trues thus counting how many true values there are.
...
> Question 1:
By using how='left', we are preserving the main dataset.
To check if any shipments were lost during the merge:
```{python}
# Check if all shipments kept their original data
original_shipments = set(clean_shipments['shipID'])
merged_shipments = set(shipments_with_category['shipID'])
print(f"All shipments preserved: {original_shipments == merged_shipments}")
```
Question 2: There would be duplicate shipments created in the merged dataframe. We would need to remove these duplicates.
...
> It breaks each the dataframe up into multiple mini dataframes, divided up by class in a particular column. This way, we can further analyze a specific group. In the example, you split by product category. This allows for analysis of each product category individually.

The .agg() function applies the specified functions to each group and combines the results. Without the .agg() function, there would be no summary statistics.

```{python}
service_by_category = (
    shipments_with_category
    .groupby(['shipID', 'prodCategory'])  # Split by product category & shipID
    .agg({
        'is_late': ['any', 'count', 'sum', 'mean'],  # Count, late count, percentage late
        'days_late': ['mean', 'max']  # Average and max days late
    })
...
```
Question 2:
By grouping by both ShipID and prodCategory, we are able to analyze how many shipments are late rather than products. This allows us to study patterns in shipments rather than products as multiple product categories could be on a single shipment. 
...
> By looking at the particular ShipIDs, we may be able to track down which particular shipments run late rather than which product types run late so that we could potentially work with the distributors to tighten the ship time. Additionally, by knowing the percentage of multi-category shipments, ZappTech could do one of the following:
1) adjust customer expectations when their goods are part of a multi-category shipment
2) try to reduce multi-category shipments and coordinate only single-cateogry shipments if this is feasible
