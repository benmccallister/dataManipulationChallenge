...
```
Question 2:
By grouping by both ShipID and prodCategory, we are able to analyze how many shipments are late rather than products. This allows us to study patterns in shipments rather than products as multiple product categories could be on a single shipment. By looking at the particular ShipIDs, we may be able to track down which particular shipments run late rather than which product types run late so that we could potentially work with the distributor.
